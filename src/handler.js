...
const getAllBooksHandler = (request, h) => {
  ...
  if (reading !== undefined) {
    const hasReading = books.filter((book) => book.reading === true);
    const response = h.response({
      status: 'success',
      data: {
        books: hasReading.map((book) => ({
          id: book.id,
          name: book.name,
  ...
  if (finished !== undefined) {
    const hasFinished = books.filter((book) => book.finished === true);
    const unFinished = books.filter((book) => book.finished === false);

    if (Number(finished) === 1) {
      const response = h.response({
        status: 'success',
        data: {
          books: hasFinished.map((book) => ({
            id: book.id,
            name: book.name,
            publisher: book.publisher,
          })),
